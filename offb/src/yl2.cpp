...
//predefined
static double gx = 3.4610 + 6 * cos(M_PI * 3/2), gy = 6 + 6 * sin(M_PI * 3/2), gz = 1.5;
static double sx = 4 * cos(M_PI * 1/2), sy = 4 * sin(M_PI * 1/2);
//!!!!!!!NEED SET TO ZEROs WHEN EXPERIMENT AS VICON DOESN'T EXIST FRAME DIFFERENCE...I THINK?
...
static UAVpose uavinfo, uavinfo_1, uavinfo_0, uavinfo_1_f, uavinfo_0_f;
...
static offb::objs devices;
static bool got;
void route_cb(const offb::objsConstPtr& msg)
{
    devices.points = msg->points;
    got = msg->got_stuff;
}
...
{
    cout<<"agent 2"<<endl;
    ros::init(argc, argv, "yl2"); //initialize ROS via passing argc argv. the "offb_node" will be the name of your node's name
    ...
    ros::Subscriber sub_state = nh.subscribe<mavros_msgs::State>
                                ("/uav2/mavros/state", 1, state_callback);


    ros::Subscriber sub_uavposition = nh.subscribe<geometry_msgs::PoseStamped>
                                   ("/uav2/mavros/local_position/pose", 1, position_callback);
    ...
    ros::Subscriber sub_velocity = nh.subscribe<geometry_msgs::TwistStamped>
                                      ("/uav2/mavros/local_position/velocity_local", 1, velocity_callback);
    ros::Subscriber sub_frame = nh.subscribe<offb::frame>
                                  ("/uav2/frame", 1, frame_cb);
    ros::Subscriber sub_route = nh.subscribe<offb::objs>
                                  ("/uav2/route", 1, route_cb);

    //publishers
    ros::Publisher pub_traj_pts = nh.advertise<geometry_msgs::PoseStamped>
                                   ("/uav2/mavros/setpoint_position/local", 1);

    ros::Publisher pub_now = nh.advertise<geometry_msgs::PoseStamped>
                                  ("/uav2/current_state", 1);
    ...
    while(ros::ok())
    {
        ...
        if(fly == Move2wp && (ros::Time::now().toSec() - last_request > ros::Duration(4.0).toSec()))
        {
            ...
            Eigen::Vector3d goal(devices.points[wp_i].X_c, devices.points[wp_i].Y_c, devices.points[wp_i].Z_c);
...
//            if(traj_v.size() != 0)
            ...
            else
            {
                fly = Hover;
                last_request = ros::Time::now().toSec();
                lep = {uavinfo.x, uavinfo.y, uavinfo.z};
            }

            if(false)
            {
                cout<<"uav2 says gonna hit"<<endl;
                fly = Cal_T;
            }
...
bool detect()
{
    double d20 = sqrt( pow( (predicted(0) - uavinfo_1.x), 2 )
                       + pow( (predicted(1) - uavinfo_1.y), 2 )
                       + pow( (predicted(2) - uavinfo_1.z), 2 ) );
    double d21 = sqrt( pow( (predicted(0) - uavinfo_0.x), 2 )
                       + pow( (predicted(1) - uavinfo_0.y), 2 )
                       + pow( (predicted(2) - uavinfo_0.z), 2 ) );
    double d20f = sqrt( pow( (predicted(0) - uavinfo_1_f.x), 2 )
                       + pow( (predicted(1) - uavinfo_1_f.y), 2 )
                       + pow( (predicted(2) - uavinfo_1_f.z), 2 ) );
    double d21f = sqrt( pow( (predicted(0) - uavinfo_0_f.x), 2 )
                       + pow( (predicted(1) - uavinfo_0_f.y), 2 )
                       + pow( (predicted(2) - uavinfo_0_f.z), 2 ) );

//    ofstream ds("/home/patrick/d_2.txt", ios::app);
//    ds<<d20<<endl;
//    ds<<d21<<endl;
//    ds<<d20f<<endl;
//    ds<<d21f<<endl;
//    ds<<endl;
//    ds.close();
    if(d20 < r_clear || d21 < r_clear || d20f < r_clear || d21f < r_clear)
    {
        if(r_safe > uavinfo.z)
...
//    cout<<traj.getTotalDuration()<<endl;
    for(double t = 0.02; t < traj.getTotalDuration(); t+=T)
    {
        ...
        ofstream save("/home/patrick/catkin_ws/src/offb/src/include/tracklog.txt",ios::app);
        save<<temp.x<<endl;
        save<<temp.y<<endl;
        save<<temp.z<<endl;
        save.close();
