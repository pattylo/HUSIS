...
//                                  ("/searched_by_yolo", 1, obj_cb);
    ...
    offb::obj p1, p2, p3, p4, p5, p6, p7, p8, p9, p10;
    p1.X_c = 3; p1.Y_c = 3; p1.Z_c = 1.5;
    p2.X_c = 3; p2.Y_c = -3; p2.Z_c = 1.5;
    p3.X_c = -2; p3.Y_c = -2; p3.Z_c = 1.5;
    p4.X_c = 4; p4.Y_c = 0; p4.Z_c = 1.5;
    p5.X_c = -3; p5.Y_c = 4; p5.Z_c = 1.4;
    p6.X_c = -0.5; p6.Y_c = 4, p6.Z_c = 1.6;
    p7.X_c = 0; p7.Y_c = -4; p7.Z_c = 1.5;
    p8.X_c = -1.5; p8.Y_c = 3; p8.Z_c = 1.2;
    p9.X_c = 1; p9.Y_c = 4; p9.Z_c = 1.4;
    p10.X_c = 2; p10.Y_c = -5; p10.Z_c = 1.5;

    devices.push_back(p1);
    devices.push_back(p2);
    devices.push_back(p3);
    devices.push_back(p4);
    devices.push_back(p5);

    devices.push_back(p6);
    devices.push_back(p7);
    devices.push_back(p8);
    devices.push_back(p9);
    devices.push_back(p10);
    ...
    while(ros::ok())
    {
        ...
        if(true && temp_i < 2)
        {
            ...
            //the result_id indicates which of the devices correspond to which agent
...
//                cout<<uav1<<endl<<endl;
//                cout<<uav2<<endl<<endl;
                for(auto what : uav0.points)
                {
                    cv::Point temp = cv::Point( (what.X_c-(-6)) * 100, (6 - what.Y_c) * 100 );
                    cv::Point temp1 = cv::Point( (what.X_c-(-6)) * 100 - 100, (6 - what.Y_c) * 100 + 24 );
                    char x[10], y[10];
                    sprintf(x, "%.2f", what.X_c);
                    sprintf(y, "%.2f", what.Y_c);
                    string p = "p";
                    string text = p +"(" + x + ", "+ y + ")";
                    cv::circle(show1, temp, 5, CV_RGB(200,0,0), -1);
                    cv::putText(show1, text, temp1,
                                cv::FONT_HERSHEY_COMPLEX_SMALL,1,CV_RGB(0,0,0));
                }
                for(auto what : uav1.points)
                {
                    cv::Point temp = cv::Point( (what.X_c-(-6)) * 100, (6 - what.Y_c) * 100 );
                    cv::Point temp1 = cv::Point( (what.X_c-(-6)) * 100 - 100, (6 - what.Y_c) * 100 + 24 );
                    char x[10], y[10];
                ...
                {
                    cv::Point temp = cv::Point( (what.X_c-(-6)) * 100, (6 - what.Y_c) * 100 );
                    cv::Point temp1 = cv::Point( (what.X_c-(-6)) * 100 - 100, (6 - what.Y_c) * 100 + 12 );
                    char x[10], y[10];
                    sprintf(x, "%.2f", what.X_c);
                    sprintf(y, "%.2f", what.Y_c);
                    string p = "p";
                    string text = p +"(" + x + ", "+ y + ")";
                    cv::circle(show2, temp, 5, CV_RGB(200,0,200), -1);
                ...
                for(auto what : uav1.points)
                {
                    ...
                    string p = "p";
                    string text = p +"(" + x + ", "+ y + ")";
                    cv::circle(show2, temp, 5, CV_RGB(0,0,200), -1);
                ...
                for(auto what : uav2.points)
                {
                    ...
                    string p = "p";
                    string text = p +"(" + x + ", "+ y + ")";
                    cv::circle(show2, temp, 5, CV_RGB(0,200,0), -1);
                ...
                for(auto what : uav0.points)
                {
                    ...
                    string p = "p";
                    string text = p +"(" + x + ", "+ y + ")";
                    cv::circle(show3, temp, 5, CV_RGB(200,0,200), -1);

                }
                for(int i = 0; i < int(uav0.points.size() - 1); i++)
                {
                    cv::Point pp1 = cv::Point( (uav0.points[i].X_c-(-6)) * 100, (6 - uav0.points[i].Y_c) * 100 );
                    cv::Point pp2 = cv::Point( (uav0.points[i + 1].X_c-(-6)) * 100, (6 - uav0.points[i + 1].Y_c) * 100 );
                    cv::line(show3, pp1, pp2, CV_RGB(0,0,0), 2);
                ...
                for(auto what : uav1.points)
                {
                    ...
                    string p = "p";
                    string text = p +"(" + x + ", "+ y + ")";
                    cv::circle(show3, temp, 5, CV_RGB(0,0,200), -1);

                }
                for(int i = 0; i < int(uav1.points.size() - 1); i++)
                {
                    cv::Point pp1 = cv::Point( (uav1.points[i].X_c-(-6)) * 100, (6 - uav1.points[i].Y_c) * 100 );
                    cv::Point pp2 = cv::Point( (uav1.points[i + 1].X_c-(-6)) * 100, (6 - uav1.points[i + 1].Y_c) * 100 );
                    cv::line(show3, pp1, pp2, CV_RGB(0,0,0), 2);
                }
                for(auto what : uav2.points)
                {
                    ...
                    string p = "p";
                    string text = p +"(" + x + ", "+ y + ")";
                    cv::circle(show3, temp, 5, CV_RGB(0,200,0), -1);

                }
                for(int i = 0; i < int(uav2.points.size() - 1); i++)
                {
                    cv::Point pp1 = cv::Point( (uav2.points[i].X_c-(-6)) * 100, (6 - uav2.points[i].Y_c) * 100 );
                    cv::Point pp2 = cv::Point( (uav2.points[i + 1].X_c-(-6)) * 100, (6 - uav2.points[i + 1].Y_c) * 100 );
                    cv::line(show3, pp1, pp2, CV_RGB(0,0,0), 2);
                }
...
//        cv::imshow("hi", show1);
//        cv::waitKey(0);
//        cv::imwrite("/home/patrick/1.png", show1);
//        cv::imshow("hi", show2);
//        cv::waitKey(0);
//        cv::imwrite("/home/patrick/2.png", show2);
//        cv::imshow("hi", show3);
//        cv::waitKey(0);
//        cv::imwrite("/home/patrick/3.png", show3);
