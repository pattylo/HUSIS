...
//predefined
static double gx = 0 + 6 * cos(M_PI * 1/6), gy = 0 + 6 * sin(M_PI * 1/6), gz = 1.5;
...
{
    cout<<"agent 0"<<endl;
    ros::init(argc, argv, "yl0"); //initialize ROS via passing argc argv. the "offb_node" will be the name of your node's name
    ...
    waypts takeoff ={1.5,0,2};
    waypts lsp = {1.5,0,0}, lep;
    vector<waypts> initializer;
    ...
    while(ros::ok())
    {
        ...
        //move~~~~~~~~~~~~~~~~~~~~~~~~~~
        if(fly == Move2wp && (ros::Time::now().toSec() - last_request > ros::Duration(5.0).toSec()))
        {
...
//        cout<<endl<<wp_i<<endl;
//        cout<<devices.points.size()<<endl<<endl;
