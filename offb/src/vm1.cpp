...
{
    cout<<"agent 1"<<endl;
    ros::init(argc, argv, "yl1"); //initialize ROS via passing argc argv. the "offb_node" will be the name of your node's name
...
//            cout<<goal<<endl<<endl;
        ...
        if(fly == Hover)
        {
            move.hover(pose, lep);/*here!!!!!!*/
            if(wp_i < devices.points.size() - 1)
            {
                cout<<"I now hover"<<endl;
                cout<<lep.x<<endl;
                cout<<lep.y<<endl;
                wp_i += 1;
                fly = Move2wp;
